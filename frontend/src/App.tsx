...
const LoadBalancerFlow = ({ stats }: { stats: LbStat[] }) => {
  const targetIps = (import.meta.env.VITE_TARGET_VPS_IPS || '').split(',').map(ip => ip.trim()).filter(Boolean);
...
function App() {
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-[#1a1c23] via-[#050505] to-[#000000] text-white font-sans px-4 pb-4 pt-2 md:px-6 md:pb-6 md:pt-3 lg:px-8 lg:pb-8 lg:pt-4">
